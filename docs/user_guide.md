...
cmake -B build -DPORTOPT_BUILD_CLI=OFF      # no `po` CLI binary
...
Two files are required. Pass the assets CSV to the CLI; `po`
...
The CLI binary is `po`. The Python module remains `portopt`; only the
user-facing command name was shortened.

```
po [global options] <subcommand> [options]
